	...
	create = function(args) 
		...
		self.draw = function(self, gContext)
			...
			if ( self.data.bgImage ~= nil ) then
				gContext:drawImage({ position=self.data.position, size=self.data.size, image=self.data.bgImage, drawMode=self.data.bgDrawMode, imageGravity=self.data.bgGravity });
			end
		...
		self.setHook = function(self, method, func)
			if ( self.plugs[method] == nil ) then
				Console.warn("[GTK] setHook: No plug for '" .. method .. "' on widget.");
				return;
			end
		...
		self.addConnector = function(self, method, target, action)
			if ( self.plugs[method] == nil ) then
				Console.warn("[GTK] addConnector: No plug for '" .. method .. "' on widget.");
				return;
			end

			self.plugs[method]:addConnector(target, action);
		end

		self.removeConnector = function(self, method, target, action)
			if ( self.plugs[method] == nil ) then 
				Console.warn("[GTK] removeConnector: No hook for '" .. method .. "' on widget.");
				return;
			end

			self.plugs[method]:removeConnector(target, action);
		end
...
GImageView = {
	create = function(args)
		...
		self.data.image = iff(args.image ~= nil, args.image, nil);
...
GBaseButton = {
	create = function(args)
		...
		if ( args.onPressed ~= nil ) then self:setHook("onPressed", args.onPressed); end
		if ( args.onReleased ~= nil ) then self:setHook("onReleased", args.onReleased); end
...
GCheckBox = {
	create = function(args)
		...
		self.doRefresh = function(self)
			if (self.data.isChecked) then
				self.imageView.data.image = self.data.checkedImage;
			else
				self.imageView.data.image = self.data.normalImage;
			end
		end
